...
namespace Deenee {
    public class DiscordBehavior : MainBehavior {
            ...
            imageManager.Fetch(Discord.ImageHandle.User(userID), (result, handle) => {
                {
                    if (result == Discord.Result.Ok) {
                        ...
                        var tex = imageManager.GetTexture(handle);
                        UnityEngine.GameObject.Find("Cube").GetComponent<UnityEngine.Renderer>().material.mainTexture = tex;
        ...
        public const long DISCORD_CLIENT_ID = 643069390966882325;
        Discord.Discord discord { get; set; }
        Discord.ApplicationManager applicationManager { get; set; }
        Discord.LobbyManager LobbyManager { get; set; }
    ...
    */
        ...
        public void Send(long lobbyId, object data) {
            BinaryFormatter bf = new BinaryFormatter();
            MemoryStream stream = new MemoryStream();
            bf.Serialize(stream, data);
            LobbyManager.SendLobbyMessage(lobbyId, stream.ToArray(), (_)=> { });
        }
        private Dictionary<Type, List<Delegate>> dataListeners = new Dictionary<Type, List<Delegate>>();
        public void AddDataListener<T>(Action<SendData<T>> func) {
            AddDataListener(typeof(T), (Delegate)func);
        }
        public void AddDataListener(Type t, Action<SendData<object>> func) {
            AddDataListener(t, (Delegate)func);
        }
        private void AddDataListener(Type t, Delegate func) {
            if (!dataListeners.ContainsKey(t)) {
                dataListeners[t] = new List<Delegate>();
            }
            dataListeners[t].Add(func);
        }
        private List<Action<Discord.Lobby>> newLobbyListener = new List<Action<Discord.Lobby>>();
        public void AddNewLobbyListener(Action<Discord.Lobby> func) {
            newLobbyListener.Add(func);
        }
    }
    [Serializable]
    public class SendData<T> {
        public Discord.User User { get; private set; }
        public T Data { get; private set; }
        public SendData(Discord.User user, T data) {
            User = user;
            Data = data;
        }
    }
